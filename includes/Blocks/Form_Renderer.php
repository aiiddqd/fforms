...
final class Form_Renderer {
	...
	private static function render_shell( int $form_id, string $content ): string {
		...
		return '<div ' . $wrapper . '><form class="fforms-form" data-wp-interactive="fforms/form" data-wp-context="' . esc_attr( $context ) . '" data-wp-on--submit="actions.submit" data-wp-bind--aria-busy="context.isSubmitting"><div class="fforms-fields">' . $content . '</div><div class="fforms-hp" aria-hidden="true"><label>' . esc_html__( 'Website', 'fforms' ) . '<input type="text" name="website" data-fforms-honeypot tabindex="-1" autocomplete="off"></label><label>' . esc_html__( 'Company', 'fforms' ) . '<input type="text" name="company" data-fforms-honeypot tabindex="-1" autocomplete="organization"></label><label>' . esc_html__( 'Phone', 'fforms' ) . '<input type="tel" name="phone" data-fforms-honeypot tabindex="-1" autocomplete="tel"></label></div><div class="fforms-response" role="status" aria-live="polite" data-wp-text="context.message" data-wp-class--is-error="context.isError"></div></form></div>';
	}
