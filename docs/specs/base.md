...
2. В боковой панели может переключить режим: в `block` редактирует форму в Gutenberg, в `headless` задаёт JSON-схему полей без блочного холста. При переключении набор полей преобразуется между блоками и схемой.
...
- режим `Block editor` или `Headless API` в боковой панели; новая форма создаётся как `Block editor`, а смена режима преобразует текущие поля между блоками и JSON-схемой;
