...
1. Пользователь создаёт запись в **FForms → Добавить форму**.
2. Выбирает тип `contact` или `lead` и задаёт поля JSON-схемой.
3. Настраивает получателей, сообщение об успехе и при необходимости автоответ.
...
- `_fforms_schema` — нормализованная JSON-схема;
...
| `GET /forms` | публичный | До 100 опубликованных форм по алфавиту. |
| `GET /forms/{id}` | публичный | Опубликованная форма, схема, сообщение об успехе и submit URL. |
...
Динамический блок `fforms/form` позволяет:
...
CSS и frontend JavaScript подключаются только когда блок реально отрендерил опубликованную форму. Стили минимальны и не зависят от конкретной темы, но отдельной интеграции с `theme.json` и Global Styles пока нет. Без JavaScript форма показывает предупреждение и не отправляется.
...
- ручное редактирование JSON-схемы;
...
Уведомление администратору включено по умолчанию. Получателей можно перечислить через запятую; при пустом значении используется `admin_email`. Автоответ отправляется на значение настроенного email-поля.
