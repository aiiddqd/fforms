...
1. Пользователь создаёт запись в **FForms → Добавить форму** и выбирает неизменяемый режим: `block` или `headless`.
2. В режиме `block` редактирует форму в Gutenberg; в режиме `headless` задаёт JSON-схему полей в панели редактора без блочного холста.
...
- выбор `Block editor` или `Headless API` до создания черновика; смена режима после создания не поддерживается;
