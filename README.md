...
**Конструктор** — форма собирается блоками в Gutenberg и после публикации доступна сразу всеми способами: блок **FForms Form**, шорткод `[fform id=123]`, а если включить «Share via link» в панели «Publication» — ещё и секретная ссылка `/forms/<токен>/` (без индексации), iframe- и js-сниппеты. Ссылку можно перевыпустить: старая перестаёт работать сразу.
